...
template <typename T> T natural_number_set<T>::pop_any()
{
    ...
    {
        throw efanna2e::ANNException("No values available", -1, __FUNCSIG__, __FILE__, __LINE__);
    }
...
{
  if (!_values_bitset || id >= _values_bitset->size()) return false;
  return _values_bitset->test(static_cast<size_t>(id));
}

// new
template <typename T> void natural_number_set<T>::print_elements() const
{
    for (const auto& value : _values_vector) {
        std::cout << value << " ";
    }
    std::cout << std::endl;
}
