...
struct Neighbor
{
    ...
    Neighbor(unsigned id, float distance) : id{id}, distance{distance}, flag(false)
    {
    }
...
class NeighborPriorityQueue
{
  public:
    ...
    bool has_unexpanded_node(size_t temp_capacity) const
    {
        return _cur < _size && _cur < temp_capacity;
    }
